...
Unlike @link["https://github.com/jyp/prettiest/"]{the canonical Haskell implementation} which removes the choice operator to improve performance, we implement all constructs described in @citet[Ber17], with additional optimizations, constructs for practical printers, and a much better handling of overflow.
...
@defproc[(memoize [f (-> any/c any/c)] [#:backend backend procedure? hasheq]) procedure?]{
  Memoizes the function @racket[f] based on the @racket[backend] data structure.
}
